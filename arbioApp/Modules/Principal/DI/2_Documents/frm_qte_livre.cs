...
namespace arbioApp.Modules.Principal.DI._2_Documents
{
    public partial class frm_qte_livre : Form
    {
        ...
        private void btn_valider_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    {
                        cmd.Connection = con;
                        cmd.CommandText = @"INSERT INTO BL VALUES(@Do_Piece,@arref,@bl)";

                        cmd.Parameters.Clear();
                        cmd.Parameters.AddWithValue("@Do_Piece", doPieceOriginal);
                        cmd.Parameters.AddWithValue("@arref", txtRef.Text);
                        cmd.Parameters.AddWithValue("@bl", txtBL.Text);

                        if (verifier_existence_bl(doPieceOriginal, txtRef.Text, txtBL.Text)){
                            MessageBox.Show("Ce BL existe déjà pour cette référence dans ce document", "Erreur",MessageBoxButtons.OK, MessageBoxIcon.Error);
                            txtBL.Text = "";
                            txtBL.Focus();
                        }
                        else {
        ...
        private bool verifier_existence_bl(String do_piece, String ar_ref, String bl)
        {
            bool b_test = false;
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string existingStock = @"
                                         SELECT TOP 1 BL
                                         FROM BL
                                         WHERE Ar_Ref = @AR_Ref AND DO_Piece = @Do_Piece AND BL=@BL";

                    String? existingBL = null;

                    using (SqlCommand checkCmd = new SqlCommand(existingStock, connection))
                    {
                        checkCmd.Parameters.AddWithValue("@AR_Ref", ar_ref);
                        checkCmd.Parameters.AddWithValue("@Do_Piece", do_piece);
                        checkCmd.Parameters.AddWithValue("@BL", bl);

                        var result = checkCmd.ExecuteScalar();
                        if (result != null && result != DBNull.Value)
                            existingBL = result.ToString();
                    }
                    if (existingBL != null)
                    {
                       b_test=true;
